...
function UpdateTodo() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://odd-rose-eagle.cyclic.app/api/todos/getusertodo/${id}`, { headers: { Authorization: `Bearer ${token}` } })
            .then(res => {
    ...
    const updateTodo = () => {
        const token = localStorage.getItem('token');
        axios.put(`https://odd-rose-eagle.cyclic.app/api/todos/updateusertodo/${id}`, { content: todo }, { headers: { Authorization: `Bearer ${token}` } })
            .then(res => {
